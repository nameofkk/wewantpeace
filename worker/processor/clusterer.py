...
MAX_EVENTS_PER_CLUSTER = 50  # 25→50 복원, 대형 사건은 50건 이상 보도됨
...
SUBTOPIC_PENALTY = 0.08  # 0.12→0.08 복원, 다른 sub_topic 적당한 분리
...
_TOPIC_FILTER_STEMS: frozenset[str] = frozenset({
    ...
    # 공통 보도 단어 — 뉴스에서 자주 나타나 가짜 유사도 생성하는 것만 제거
    # (attack/kill/strike 등은 사건 구별에 유용하므로 유지)
    "secur", "crisis",
})
...
def _ai_same_event(
    ...
    try:
        ...
        resp = client.chat.completions.create(
            ...
            # 하나 뱉기 전에 reasoning 토큰을 먼저 쓴다. 2026-09-16 배포 직후
            # 프로덕션 로그에서 max_tokens=250도 부족해 json_validate_failed가
            # 재현됨(classify/ai_title과 같은 패턴) — 동일하게 넉넉히 잡는다.
            max_tokens=1200,
...
def _fix_translation_style(text: str) -> str:
    ...
    # _TRANS_END_RE의 다[']고... 패턴이 따옴표를 소비해 오픈 따옴표만 남는 버그 방지
    prepped = re.sub(
        r"다(['\u2018\u2019\u201c\u201d\u02bc])\s*고\s+"
        r"(?:밝혔습니다|합니다|됩니다|했습니다|말했습니다|전했습니다)[.！？]?\s*$",
        r'다\1',
        prepped,
    )
    ...
    # 7단계: 잔류 인용절 보고주체 정리
    # 例) "회수됐다고 미군" → "회수됐다"
    # 例) "증가하고 있다고 경고" → "증가하고 있다"
    # 보고동사 제거(1단계) 후 남은 "X다/라고 [보고주체명사]" 패턴 제거
    fixed = re.sub(
        r'([다라])고(?:\s+[가-힣·\-]{1,8}){1,4}\s*$',
        r'\1',
        fixed,
    )
    ...
    *,
    ...
    for small in small_clusters:
        ...
        for target in targets:
            ...
            # 병합용 임계값 (0.12) — AI 호출 제거 (DB 트랜잭션 내 외부 API 호출 금지)
            if sim >= 0.12 and sim > best_sim:
                best_sim = sim
                best_target = target
